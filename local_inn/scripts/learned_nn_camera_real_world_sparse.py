...
#tained_model_weights_path = '/home/siva/cps_280_ws/src/local_inn/scripts/real_world_trained_models/run3_single_model_with_stddev_0.5/proj_cameralocal_INN/local_INN_model_best.pt'   #'local_INN_model_best_doulbe_latent.pt'


tained_model_weights_path_X =     '/media/siva/Expansion/MTech_Thesis/real_word_exp_INN/real_world_trained_model16thmay/X/proj_cameralocal_INN/local_INN_model_best.pt'
tained_model_weights_path_Y =     '/media/siva/Expansion/MTech_Thesis/real_word_exp_INN/real_world_trained_model16thmay/Y/proj_cameralocal_INN/local_INN_model_best.pt'
tained_model_weights_path_theta = '/media/siva/Expansion/MTech_Thesis/real_word_exp_INN/real_world_trained_model16thmay/YAW/proj_cameralocal_INN/local_INN_model_best.pt'
...
class VariationalEncoder(nn.Module):
    ...
    def forward(self, x, random_nums):
        ...
        # Create CSV writers if this is the first print
        mu_file = 'encoded_mu_values.csv'
        sigma_file = 'encoded_sigma_values.csv'

        mu_np = mu.detach().cpu().numpy()
        sigma_np = sigma.detach().cpu().numpy()

        # Append mode
        with open(mu_file, 'a', newline='') as f_mu, \
            open(sigma_file, 'a', newline='') as f_sigma:
            
            mu_writer = csv.writer(f_mu)
            sigma_writer = csv.writer(f_sigma)

            for mu_row, sigma_row in zip(mu_np, sigma_np):
                mu_writer.writerow(mu_row)
                sigma_writer.writerow(sigma_row)

        # with open('encoded_sigma_values.txt', 'a') as f:
        #     f.write(str(print_count))
        #     f.write("\n\n")
        #     #f.write("sigma shape: {}\n".format(tuple(sigma.shape)))
        #     f.write("sigma values:\n")
        #     f.write(str(sigma.detach().cpu().numpy().tolist()))
        #     f.write("\n\n")


        # with open('encoded_mu_values.txt', 'a') as f:
        #     f.write(str(print_count))
        #     f.write("\n\n")
        #     #f.write("mu shape: {}\n".format(tuple(mu.shape)))
        #     f.write("mu values:\n")
        #     f.write(str(mu.detach().cpu().numpy().tolist()))
        #     f.write("\n\n")
...
class Local_INN_TRT_Runtime():
    def __init__(self, sample_num, tained_model_weights_path):
...
local_inn_X = Local_INN_TRT_Runtime(INFERENCE_SAMPLES, tained_model_weights_path_X)
local_inn_Y = Local_INN_TRT_Runtime(INFERENCE_SAMPLES, tained_model_weights_path_Y )
local_inn_Yaw = Local_INN_TRT_Runtime(INFERENCE_SAMPLES, tained_model_weights_path_theta )
...
def predict_from_model(prev_pose):
    ...
    ##############################
    global image_id
    #image_folder = '/home/siva/scene_clutter_experiment/changed_imgs_sift_removed_radius_0.5'
    #image_folder = '/home/siva/scene_clutter_experiment/original_images'
    #image_folder = '/home/siva/scene_clutter_experiment/changed_imgs_sift_avgregion_radius_0.5'
    #image_folder = '/home/siva/scene_clutter_experiment/scen_change_Z_purturbations'

    image_folder = '/media/siva/Expansion/MTech_Thesis/real_world_data_collection_15thmay/test_path_images'

    rospy.loginfo(f"current image number is:{image_id}")
    image_path = os.path.join(image_folder, f"vid1_{image_id}.jpg")
    ...
    inferred_state_yaw, inferred_states_yaw = local_inn_Yaw.reverse(normalised_img_tensor, encode_cond_pose)
    inferred_state_x, inferred_states_x = local_inn_X.reverse(normalised_img_tensor, encode_cond_pose)
    inferred_state_y, inferred_states_y = local_inn_Y.reverse(normalised_img_tensor, encode_cond_pose)
    

    inferred_state_final = inferred_state_x
    inferred_state_final[1] = inferred_state_y[1]
    inferred_state_final[2]= inferred_state_yaw[2]

    cv2.destroyAllWindows()
    return inferred_state_final
