    ...
    while(not trainer.is_done()):
        ...
        for (images, poses, cond_poses) in train_loader:
            ...
            with autocast(enabled=mix_precision):
                ...
                loss_reverse = l1_loss(x_hat_0[:, [4, 5, 10, 11]], x_hat_gt[:, [4, 5, 10, 11]]) 
                ...
                x_hat_i_loss = torch.mean(torch.min(torch.mean(torch.abs(x_hat_i[:, :, [4, 5, 10, 11]] - x_hat_gt[:, [4, 5, 10, 11]]), dim=2), dim=0)[0])
        ...
        with torch.no_grad():
            for (images, poses, cond_poses) in test_loader:
                ...
                with autocast(enabled=mix_precision):
                    ...
                    epoch_posit_err.append(torch.median(torch.norm(result_posit[:, 0:2] - result_posit[:, 0:2], dim=1)))
